...
export const PerfilView = () => {
  ...
  const handleSaveProfile = async () => {
    setLoading(true);
    try {
      // 1. Guardamos datos del perfil en la tabla profiles
      ...
      setIsEditing(false);
      setStatus({ msg: 'Perfil y credenciales actualizados.', type: 'success' });
    } catch (e) {
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 4, borderRadius: 4, textAlign: 'center', border: '1px solid #eee', boxShadow: 'none' }}>
            <Avatar sx={{ width: 100, height: 100, mx: 'auto', mb: 2, bgcolor: '#5A189A', fontSize: 40 }}>{user?.nombre[0]}</Avatar>
            {isEditing ? (
              <Stack spacing={2}>
            ...
            <Stack spacing={2} textAlign="left">
              <Stack direction="row" spacing={2}><Badge color="action" /><Box><Typography variant="caption" color="textSecondary">DNI</Typography>
              {isEditing ? <TextField size="small" variant="standard" value={formData.dni} onChange={(e) => setFormData({...formData, dni: e.target.value})} /> : <Typography variant="body2" sx={{ fontWeight: 700 }}>{user?.dni}</Typography>}</Box></Stack>
              
              <Stack direction="row" spacing={2}><Cake color="action" /><Box><Typography variant="caption" color="textSecondary">Nacimiento</Typography>
              {isEditing ? <TextField type="date" size="small" variant="standard" value={formData.fecha_nacimiento} onChange={(e) => setFormData({...formData, fecha_nacimiento: e.target.value})} /> : <Typography variant="body2" sx={{ fontWeight: 700 }}>{user?.fecha_nacimiento || 'No cargada'}</Typography>}</Box></Stack>
            </Stack>
          </Paper>
        </Grid>
