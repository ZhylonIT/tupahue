...
export const DocumentacionView = ({ hijo, onUpdateScout }) => {
  ...
  // 🎯 MODIFICADO: Ya no cierra los modales automáticamente
  const handleSaveForm = (hijoActualizado, docId) => {
    const nuevosDocs = [...new Set([...(hijoActualizado.documentos || []), docId])];
    ...
    // Quitamos los setOpen(false) de acá. 
    // Ahora cada Form decide cuándo cerrarse mediante su botón "Cerrar".
  };
    ...
    <Box sx={{ animation: 'fadeIn 0.3s' }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 4 }}>
        <Box>
          ...
          <Typography variant="subtitle1" color="textSecondary">
            Protagonista: <b style={{ color: VIOLETA_SCOUT }}>{hijo.nombre} {hijo.apellido}</b>
          </Typography>
        </Box>
      ...
      <Grid container spacing={4}>
        <Grid item xs={12} md={7}>
          ...
          <Box>
            <Typography variant="h6" sx={{ fontWeight: 800, mb: 2, color: '#455a64' }}>Documentación de Ingreso</Typography>
        ...
        <Grid item xs={12} md={5}>
          ...
          <Paper sx={{ p: 4, borderRadius: 5, textAlign: 'center', border: '1px solid #eee', bgcolor: '#fcfcfc' }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 800, mb: 3 }}>Progreso de Documentación</Typography>
            ...
            <Typography variant="body2" color="textSecondary" sx={{ mt: 3, fontWeight: 600 }}>
              {porcentaje === 100 ? "¡Legajo completo! Buen trabajo." : "Faltan documentos obligatorios."}
            </Typography>
          </Paper>
        </Grid>
      </Grid>
